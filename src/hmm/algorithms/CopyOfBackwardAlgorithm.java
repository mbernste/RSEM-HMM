...
public class CopyOfBackwardAlgorithm 
{	
			...
			for (int j = sortedSilent.size() - 1; j >= 0; j--)
			{
				...
				for (State forwardState : model.getStates())
				{
					...
					if (forwardState.isSilent()) // Transitions to silent states
					{
						...
						tProb  = LogTransforms.eLn(model.getTransitionProb( currState.getId(),
															 forwardState.getId() 
															  ));
					}
					else  // Transitions to non-silent states
					{
						eProb = LogTransforms.eLn(model.getEmissionProb(forwardState.getId(),
